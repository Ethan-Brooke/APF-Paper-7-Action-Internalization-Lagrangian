# Reviewers' Guide — APF Paper 7: Action, Internalization, and the Lagrangian

This guide is the physics-first walkthrough for peer reviewers of Action, Internalization, and the Lagrangian. It addresses the structural assumptions, anticipated objections, and falsification surfaces of the paper's argument. The intended audience is a peer reviewer with a background in physics or mathematics.

The guide complements three other verification routes:
- The [executable codebase](apf/) (run `python run_checks.py` for the full 21-theorem verification);
- The [Colab notebook](APF_Reviewer_Walkthrough.ipynb) (zero-install, theorem-by-theorem cells with prose + code);
- The [interactive derivation DAG](https://ethan-brooke.github.io/Action-Internalization-Lagrangian/) (hover-for-details, click-for-dependencies, animated verification).

---

## The derivation in 21 steps

The full chain from the foundational commitments to this paper's results:

**Step 1 — T_eps** (`check_T_eps`)
...
**Step 4 — T0_4_prime_BFS** (`check_T0_4_prime_BFS`)
...
**Paper 7 additionally relies on three hypotheses, flagged at point of use:**

- **H1 — Spectral-triple ansatz.** The admissibility structure admits a spectral-triple representation $(A, H, D)$ with finite-dimensional algebra and Dirac-operator spectrum encoding cost.
- **H2 — Lipschitz cost regularity.** The cost functional $\varepsilon$ is Lipschitz on the admissible set, uniformly in the capacity parameter. Required for the Seeley–DeWitt expansion to commute with the argmin operator.
- **H3 — HKM future-distinguishing causal order.** The admissible causal structure is future-distinguishing. Required for the HKM theorem to apply to admissibility-induced causal-order.
...
To prevent scope inflation, the following are flagged explicitly as *outside* the scope of Action Internalization Lagrangian:
...
- **`apf/core.py`** — the 21 theorem check functions for this paper. Each function constructs a mathematical witness, verifies the theorem's claim, and returns a structured result with name, dependencies, status, and key result.
...
This repository is the executable mathematical appendix to APF Paper 7. The canonical archival deposit is at [https://doi.org/10.5281/zenodo.18439513](https://doi.org/10.5281/zenodo.18439513) (DOI: 10.5281/zenodo.18439513).
