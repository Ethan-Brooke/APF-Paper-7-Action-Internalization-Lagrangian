...
You are reading the companion to **Paper 7** (this repo: https://github.com/Ethan-Brooke/APF-Paper-7-Action-Internalization-Lagrangian). The canonical computational engine lives at codebase **v6.9** ([Zenodo](https://doi.org/10.5281/zenodo.18604548)). Each paper has its own companion repo following the same layout.
...
Expected output: `Paper 7 (Action, Internalization, and the Lagrangian): N passed, 0 failed, N total — verified in under 1s`.
...
- What this repo is (Paper 7 + vendored `apf/` subset)
...
Not mandatory to read end-to-end, but make sure you know it exists and how to query it. The full bank has 342 entries; this paper bundles **21 of 342** for local execution. Each entry has name, module, epistemic tag, dependencies, and one-line summary.
...
- **`Paper_7_Action_Internalization_Lagrangian_v2.0.pdf`** — the manuscript. Read §1 (Introduction) and whichever section is relevant to your task. Don't try to read the whole paper before starting work.
- **`Paper_7_Action_Internalization_Lagrangian_Supplement_*.pdf`** (if present) — the Technical Supplement. Canonical proofs. Dense; dip in only where you need depth.
