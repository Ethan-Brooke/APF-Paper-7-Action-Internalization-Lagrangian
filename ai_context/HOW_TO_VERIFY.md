...
Open the interactive DAG at `https://ethan-brooke.github.io/APF-Paper-7-Action-Internalization-Lagrangian/` (requires GitHub Pages enabled on this repo's `/docs` folder). Hover any node for its claim, epistemic tag, and shortest derivation chain. Click **Run Checks** to watch all theorems verify in topological order.
...
Expected output: one line per check, then a summary `Paper 7 (Action, Internalization, and the Lagrangian): N passed, 0 failed, N total`. Elapsed time is typically under a second.
...
```bash
python -c "from apf import core; r = core.check_T_eps(); print(r)"
```
...
Every `\coderef{check_X}{module.py}` in the paper source (`Paper_7_Action_Internalization_Lagrangian_v2.0.tex`) points to a specific check function. To verify the paper's claim at a particular point:

```bash
# find all coderefs in the paper
grep -o '\\coderef{[^}]*}{[^}]*}' Paper_7_Action_Internalization_Lagrangian_v2.0.tex
